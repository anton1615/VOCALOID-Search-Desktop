# playback-session-boundary Specification

## Purpose
Define playback identity and player-session rules for explicit same-video selection across `Search`, `History`, and `WatchLater`, ensuring that cross-list rebinding creates a distinct playback session and that reconstruction paths preserve the latest list-bound playback ownership.
## Requirements
