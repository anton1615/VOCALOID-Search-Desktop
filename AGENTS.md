...
17. **PiP regain staged UX**：關閉 PiP 回主視窗時不要只 refresh snapshot；要重走與列表點擊一致的 staged enrichment flow（player first → Rust enrichment → playback-video-updated → metadata shown）
18. **pending shell 分流策略**：full mode 需要 header pending frame 來避免播放器跳動；compact mode 要用自己的 pending frame，不能沿用 full mode 外層高度，否則會重現 PiP 上方空白帶
19. **source-contract 測試維護**：當 title/frame 或 shell 策略改成 mode-specific 時，優先更新 source-contract 測試去驗證新契約，不要沿用舊的 class/CSS literal 假設
20. **Tauri 資產刷新驗證**：桌面 UI 若看起來沒反映前端改動，先確認最新 `dist` 已被 `npm run tauri build -- --debug` 重新打進新的 desktop exe
...
26. **single-video metadata source 契約**：Search 播放時保留 `videos.db` 的 shared fields（含 counters / tags / upload date / uploader_id），只用 `watch/{id}?responseType=json` 補 `description` 與 `uploader_name`；History / Watch Later 播放時由 watch JSON 提供 shared metadata，且 upload date 不可 fallback 成 watched/added timestamp
...
**效益**：
- 切換可見 list 不會再隱式中斷目前播放
- 非 active list 的 refresh / clear 不會誤清空 player
- 主視窗與 PiP 對 playback 清除事件維持一致語意
- browsing state restoration 與 playback ownership 的責任分界更清楚

**參見**：`openspec/changes/archive/2026-03-19-decouple-browsing-from-playback/`

---

### 為什麼 PiP compact header 要保留固定外殼？(2026-03-21)

**問題**：PiP compact metadata header 若直接讓內容高度驅動外層 shell，短標題與長標題會讓播放器位置跳動；若只在內層改樣式而不先定位責任層，又容易誤判是 `VideoMetaPanel`、`UnifiedPlayer` shell 或 PiP section stack 在撐高。

**決策**：
- 先用臨時 shell debug marker 區分 header / player / details 責任層
- 確認責任層後只調整 compact header shell 的固定高度 token
- compact header 內部仍以 rendered line-state 合約做 single-line / two-line 重平衡

**效益**：
- 先定位外層責任再微調 token，避免反覆改錯層
- PiP compact header 高度可穩定收斂而不影響播放器位置
- source-contract 測試可同時覆蓋 line-state 合約與 shell 層責任

**參見**：`openspec/changes/archive/2026-03-21-pip-compact-header-line-balance/`
...
| 日期 | 變更名稱 | 說明 |
|------|----------|------|
| 2026-03-21 | pip-compact-header-line-balance | PiP compact header 以 rendered line-state 在固定 shell 內重平衡，並先用 shell marker 定位責任層 |
| 2026-03-19 | decouple-browsing-from-playback | 解耦 browsing 與 playback，避免非 active list refresh 誤清空 player |
